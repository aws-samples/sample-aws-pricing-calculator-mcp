...
/**
 * Map friendly config keys to real field IDs, and fix fileSize units.
 *
 ...
 * made FSx for Lustre storage render $0 despite a valid 1200 GB config).
 *
 * This function rewrites any component key that is NOT already a valid field id
 * to the field whose id or label normalizes to the same token. It also repairs
 * fileSize units supplied as a bare size ("gb") to the field's full
 * size|frequency form ("gb|NA"), which the engine requires.
 *
 * Runs BEFORE fillRequiredDefaults/normalizeDropdownValues/addDefaultUnits so
 * those steps operate on real field ids.
 */
function normalizeFieldKeys(components, definition, templateId) {
  ...
  const validIds = new Set(activeFields.map(f => f.id));
  const byNorm = new Map();
  for (const f of activeFields) {
    // Prefer id matches; register label as a fallback alias.
  ...
  for (const [k, v] of Object.entries(components)) {
    let field = validIds.has(k) ? activeFields.find(f => f.id === k) : byNorm.get(_normKey(k));
    const targetId = field ? field.id : k;

    // Repair fileSize unit: engine expects "size|frequency" (e.g. "gb|NA"),
    // not a bare "gb". Use the field's defaultUnit when the supplied unit is a
    // bare size or missing entirely.
    if (field && field.type === 'fileSize' && v && typeof v === 'object' && field.defaultUnit) {
      const unit = v.unit;
        ...
        if (svc.subType === 'subService' && subServiceParent.has(svcKey)) {
          ...
          const components = addDefaultUnits(
            normalizeDropdownValues(
              await fillRequiredDefaults(
                normalizeFieldKeys(wrapValues(config), subDef, subTemplateId),
  ...
  async _buildServiceConfig(manifest, service, config, partition) {
    ...
    const components = this._isEC2(service)
      ...
      : addDefaultUnits(
          normalizeDropdownValues(
            await fillRequiredDefaults(
              normalizeFieldKeys(wrapValues(config), def, estimateFor),
              def, estimateFor
            ),
            def, estimateFor
          ), def, estimateFor
