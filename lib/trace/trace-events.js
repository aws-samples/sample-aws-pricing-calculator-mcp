...
//
// `tool.call` and `tool.result` are emitted by lib/trace/trace-logger.js's
...
// HTTP request, then zero or more save.retry, then exactly one of
// save.ok or save.fail.
//
// save.retry is the audit trail for a deliberate trade-off: the save POST
// is not idempotent (each call mints a fresh estimateId), so a network
// error raised after the lambda already accepted the body is ambiguous
// and the re-attempt may leave a duplicate blob server-side. Each
// save.retry means "an orphaned estimate may exist" — the only signal
// that makes those blobs attributable after the fact.
const save = {
  send: (payload) => emit('save.send', withSession(payload)),
  retry: (payload) => emit('save.retry', withSession(payload)),
...
// statuses (read-only / required-input / ok); see lib/lint/can-rehydrate.js.
