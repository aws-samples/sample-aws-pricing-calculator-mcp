 ...
 *   - Retries are opt-out per call (`retry: false`) and the attempt
 *     count is per-call, so a non-idempotent request can take a smaller
 *     budget than the default. See saveEstimate in aws-client.js for
 *     the one place that does, and why.
 *   - `onRetry` fires before each re-attempt so a non-idempotent caller
 *     can record that a duplicate may now exist server-side. It is
 *     observability, not control flow: throwing from it is not
 *     supported and would surface as the request's failure.
 */
 ...
 * @param {Function} [opts.onRetry]       ({ attempt, error }) before each re-attempt
 */
