...
// Upstream 1.2.9 decision (merged 2026-08-13): an OMITTED term defaults
// to '3 Year' — the deeper-commitment, cheaper Savings Plan. A term
// that is PRESENT but unresolvable still fails fast (stricter than
// upstream, which silently defaulted those to 3 Year too).
function resolveTerm(raw) {
  if (raw == null || raw === '') return '3 Year';
...
function parseString(str) {
  ...
  if (m) {
    ...
    return {
      ...
      term: m[1] ? (m[1] === '3' ? '3 Year' : '1 Year') : '3 Year',
  ...
  return {
    model,
    // Omitted term → '3 Year' (upstream 1.2.9 default); explicit 1 wins.
    term: termMatch ? (termMatch[1] === '3' ? '3 Year' : '1 Year') : '3 Year',
    upfrontPayment,
  };
}
...
function transformConfig(config) {
  ...
  // Dedicated Host (upstream 1.2.5): host-tenancy estimates use the DH
  // variants of the EBS storage fields. Explicit *DH fields win; when
  // tenancy is 'host', regular storage fields are promoted to their DH
  // variants and suppressed from the regular slots.
  ...
  return {
    ...
    // dataTransferForEC2 is injected by lib/mcp/handler-helpers.js#applyDefaultFields
