 ...
 *   - lib/mcp/handler-helpers.js (get_service_fields path) — agents see the
 *     synthetic field in the field list.
 *   - lib/lint/validation.js (validateConfigKeys path) — the synthetic id is
 *     accepted as a valid config key, not auto-corrected away.
 *
 * The transform side (lib/aws/ec2.js) reads top-level config.utilization
...
/**
 * Metadata merged INTO existing manifest fields (vs SYNTHETIC_BY_SERVICE
 * which appends new ones). Agent report 2026-08-12: the pricingStrategy
 * enum lived only in catalog.traps prose, where agents missed it and
 * sent display labels ("EC2 Instance Savings Plans") that silently
 * priced as On-Demand. Surfacing valid values directly on the field
 * definition — the way dropdown options appear — makes them
 * un-missable at get_service_fields time. The keys are informational
 * (validModels etc., not `options`) so the generic dropdown validator
 * never misfires on the composite type.
 */
const ENRICH_BY_SERVICE = {
  ec2Enhancement: {
    pricingStrategy: {
      helpText:
        'Pricing model. Accepts a string shorthand ("ondemand", "spot", ' +
        '"instanceSavings3yrAllUpfront") or an object ' +
        '{"model":"instanceSavings","term":"3 Year","upfrontPayment":"None"}. ' +
        'Display labels like "EC2 Instance Savings Plans" are also accepted. ' +
        'Invalid values are rejected at add_service — nothing silently ' +
        'falls back to On-Demand. Note: reserved/convertible require ' +
        'tenancy "dedicated" or "host"; under shared tenancy they are ' +
        'remapped to instanceSavings/computeSavings.',
      validModels: ['ondemand', 'instanceSavings', 'computeSavings', 'reserved', 'convertible', 'spot'],
      validTerms: ['1 Year', '3 Year'],
      validUpfrontPayments: ['None', 'Partial', 'All'],
    },
  },
};
...
function synthesizeAgentFields(serviceCode, fields) {
  ...
  const enrichments = ENRICH_BY_SERVICE[serviceCode];
  if ((!additions || additions.length === 0) && !enrichments) return fields;

  let out = fields;
  if (enrichments) {
    let touched = false;
    const mapped = fields.map(f => {
      const extra = enrichments[f.id];
      if (!extra) return f;
      touched = true;
      return { ...f, ...extra };
    });
    if (touched) out = mapped;
  }
  if (additions && additions.length > 0) {
    // De-dupe in the unlikely case the manifest already exposes the id.
    const existing = new Set(out.map(f => f.id));
    const fresh = additions.filter(a => !existing.has(a.id));
    if (fresh.length > 0) out = out === fields ? [...fields, ...fresh] : [...out, ...fresh];
  }
  return out;
}
