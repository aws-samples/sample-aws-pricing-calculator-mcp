...
// so the builder must translate the selector values the agent supplies into
// their stored forms or the estimate rehydrates read-only ($0). Returns null
// when no columnFormIPM component carries a remap block.
function columnFormRemap(def) {
  const templates = def?.templates;
  if (!Array.isArray(templates)) return null;
  for (const t of templates) {
    for (const card of (t?.cards || def?.cards || [])) {
      for (const c of (card?.inputSection?.components || [])) {
        if ((c.subType === 'columnFormIPM' || c.type === 'columnFormIPM') && c.remap?.keyValue) {
          return c.remap.keyValue;
        }
      }
    }
  }
...
// Apply a remap.keyValue map to the cell values inside a config's
// columnFormIPM field, in place. Plain cells store the value at
// cell.value (a string); utilization cells store it at
// cell.value.selectedId. A value is remapped only when it is a KEY in the
// map (selector form); already-stored target values are left alone so
// applying the remap twice is safe.
function applyColumnFormRemap(config, keyValue) {
  if (!keyValue) return;
  const remap = (v) => (typeof v === 'string' && v in keyValue) ? keyValue[v] : v;
    ...
    for (const row of rows) {
      ...
      for (const cell of Object.values(row)) {
        ...
        if (cell.value && typeof cell.value === 'object') {
          cell.value.selectedId = remap(cell.value.selectedId);
        } else {
          cell.value = remap(cell.value);
        }
      }
    }
  }
...
class EstimateBuilder {
  ...
  async toAWSPayload() {
    ...
    const buildEntries = async (serviceMap) => {
      const out = {};
      // Multiple subservice children that share a parent AND an instance
      // (e.g. AppSync's appSyncApiCall + appSyncCaching under awsAppSync)
      // collapse into a single parent envelope with a subServices[] array,
      // not one parent per child. Track envelopes by parent service code +
      // instance so siblings sharing an instance append to the existing
      // array while distinct instances get their own card (issue #37).
      const parentEnvelopes = new Map();

      for (const [compositeKey, config] of Object.entries(serviceMap)) {
        // compositeKey is `service` or `service:instance` (the instance
        // suffix is the caller's per-line-item discriminator, set via the
        // `instance` field or auto-appended from `description` on a repeat).
        // Split on the FIRST colon only so instance values may themselves
        // contain colons.
        const colonIdx = compositeKey.indexOf(':');
        const svcKey = colonIdx === -1 ? compositeKey : compositeKey.slice(0, colonIdx);
        const instanceSuffix = colonIdx === -1 ? '' : compositeKey.slice(colonIdx + 1);
        ...
        if (svc.subType === 'subService' && subServiceParent.has(svcKey)) {
          ...
          // Envelope identity = parent service + instance. Children that
          // share an instance (including the common no-instance case)
          // collapse into ONE card — e.g. AppSync's appSyncApiCall +
          // appSyncCaching, or EFS + EBS rows in a single AWS Backup card.
          // Distinct instances produce distinct cards, so the caller can
          // reproduce N separate cards of the same family (issue #37: two
          // EBS backup vaults, several S3 buckets, TGW + VPN) by giving each
          // line item a unique `instance`. Pre-fix this was keyed by
          // parent.key alone, which force-merged every sibling into one card.
          const envelopeKey = `${parent.key}\u0000${instanceSuffix}`;
          let envelope = parentEnvelopes.get(envelopeKey);
          if (!envelope) {
            ...
            out[envKey] = envelope;
            parentEnvelopes.set(envelopeKey, envelope);
          }
