...
const { resilientFetch } = require('./fetch-resilience');
...
async function fetchMappingMetadata(mappingDef) {
  ...
  const promise = (async () => {
    try {
      const res = await resilientFetch(resolved);
      if (!res.ok) return null;
...
async function loadSelectorAggregations(definition, columnFormField, region) {
  ...
  const promise = (async () => {
    try {
      const res = await resilientFetch(aggUrl);
      if (!res.ok) return null;
      const data = await res.json();
...
async function fetchServiceDefinition(manifest, serviceCode, partition = 'aws') {
  ...
  const res = await resilientFetch(`${base}${cdnPrefix}${urlPath}`);
...
async function saveEstimate(payload, { estimateId, partition } = {}) {
  ...
  // This POST is not idempotent — every call mints a fresh estimateId — so
  // a network error raised after the lambda already accepted the body is
  // ambiguous, and the re-attempt may leave a duplicate blob behind.
  //
  // We retry anyway, on a smaller budget than the default 3. Issue #7's
  // reported symptom was export_estimate failing, i.e. this exact call, so
  // excluding it would have protected the read paths and left the one that
  // actually broke unprotected. The cost of being wrong is asymmetric: the
  // duplicate is unreachable (nobody holds its URL), belongs to no
  // estimate, and inflates no cost — unlike a duplicate add_service entry,
  // which silently inflates the total. A save that fails on a transient
  // reset is a real user-visible failure. Cheap orphan beats lost save.
  //
  // Each re-attempt emits save.retry so the orphans stay attributable.
  const res = await resilientFetch(saveUrl, {
    method: 'POST',
    headers: { 'content-type': 'application/json', 'Referer': referer },
    body: jsonBody,
  }, {
    maxAttempts: 2,
    onRetry: ({ attempt, error }) => traceEvents.save.retry({
      attempt,
      code: error?.cause?.code || error?.code || error?.name,
      mayHaveOrphaned: true,
      estimateId,
    }),
  });
          ...
          if (obj.options) {
            field.options = obj.options
              ...
              .map(o => {
                ...
                // Throughput options carry a `value` property that is the
                // actual unit string the calculator stores (e.g. "mbps").
                // Expose it so agents use the storage unit, not the UI id.
...
async function fetchEstimate(estimateId, { partition } = {}) {
  ...
  const res = await resilientFetch(`${readBase}/${estimateId}`);
