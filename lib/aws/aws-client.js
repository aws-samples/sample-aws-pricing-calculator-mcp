...
async function saveEstimate(payload, { estimateId, partition } = {}) {
  ...
  // retry:false — this POST is not idempotent. Every call mints a fresh
  // estimateId, so an ECONNRESET raised after the lambda already accepted
  // the body is ambiguous, and a retry would orphan a duplicate estimate.
  // The timeout still applies, so a stalled save fails instead of hanging.
