...
function PREDICATE_HINT(predicate, failure, catalog, childToParent) {
  ...
  if (predicate === 'ec2-pricing-invalid-value') {
    const invalid = Array.isArray(ctx.invalid) ? ctx.invalid.join('; ') : '';
    return `Service ${svc}: the saved blob's pricingStrategy carries ` +
           `invalid ${invalid}. The calculator will NOT honor these values — ` +
           `it silently prices as On-Demand / 1 Year. This blob was built ` +
           `outside the normal add_service path (add_service rejects these ` +
           `values). Recovery: re-add the service via add_service with ` +
           `pricingStrategy in the accepted form, e.g. ` +
 ...
 *
 * `catalog` is the Map produced by lib/lint/catalog.js#loadCatalog. Optional:
